import {
   ...
   UPLOADER_SIGN_OUT,
   UPLOADER_SIGN_UP,
} from '../actionTypes/uploaderActionTypes';
import { SERVER_BASE_URL } from '../../config';
import {
   CREATE_COURSE,
   ADD_VIDEOS,
} from '../actionTypes/coursesActionTypes';
...
export const uploaderSignup = (user) => async (dispatch) => {
   try {
      ...
      const { data } = await axios.post(
         `${SERVER_BASE_URL}/uploader/signUp`,
         user,
         {
            headers,
         }
...
export const uploaderSignin = (user) => async (dispatch) => {
   try {
      ...
      const { data } = await axios.post(
         `${SERVER_BASE_URL}/uploader/signIn`,
         user,
         {
            headers: headers,
         }
...
export const uploaderSignout = () => async (dispatch, getState) => {
   try {
      ...
      const { data } = await axios.delete(
         `${SERVER_BASE_URL}/uploader/signOut`,
         {
            headers: headers,
         }
      );
...
export const addVideos = (courseId) => async (dispatch, getState) => {
   try {
      ...
      const { data } = await axios.post(
         `${SERVER_BASE_URL}/addVideo/${courseId}`,
         {
            headers: headers,
         }
      );
